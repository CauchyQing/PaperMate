...
import type { Paper, Tag, CategoryGroup, CategoryType } from '../../shared/types/category';
...
interface CategoryState {
  ...
  importPaper: (workspacePath: string, filePath: string, relativePath: string) => Promise<void>;

  // Paper actions
  updatePaper: (workspacePath: string, paperId: string, updates: Partial<Paper>) => Promise<void>;

  // Tag actions
  loadTags: (workspacePath: string) => Promise<void>;
  addTag: (workspacePath: string, tag: Omit<Tag, 'id' | 'createdAt'>) => Promise<void>;
  deleteTag: (workspacePath: string, tagId: string) => Promise<void>;
  assignTagToPaper: (workspacePath: string, paperId: string, tagId: string) => Promise<void>;
  removeTagFromPaper: (workspacePath: string, paperId: string, tagId: string) => Promise<void>;
}
...
export const useCategoryStore = create<CategoryState>()((set, get) => ({
  ...
  importPaper: async (workspacePath: string, filePath: string, relativePath: string) => {
    try {
      const paper = await window.electronAPI.paperImport(workspacePath, filePath, relativePath);
  ...
  updatePaper: async (workspacePath: string, paperId: string, updates: Partial<Paper>) => {
    try {
  ...
  addTag: async (workspacePath: string, tag: Omit<Tag, 'id' | 'createdAt'>) => {
    try {
  ...
  assignTagToPaper: async (workspacePath: string, paperId: string, tagId: string) => {
  ...
  removeTagFromPaper: async (workspacePath: string, paperId: string, tagId: string) => {
    const paper = get().papers.find((p) => p.id === paperId);
    if (!paper) return;
